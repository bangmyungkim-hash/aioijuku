            ...
            <div className="space-y-3">
              {links.map((link) => (
                <div key={link.student_user_id} className="card flex items-center justify-between">
                  <div>
                    <p className="font-bold text-gray-800">{studentMap[link.student_user_id] || "不明"}</p>
                    <p className="text-xs text-gray-500 mt-0.5">学習状況を確認する</p>
                  </div>
                  <a href={"/parent/student/" + link.student_user_id} className="btn-secondary text-sm py-2 px-4">詳細</a>
                </div>
              ))}
            </div>
          ) : (
