    ...
    <div className="min-h-screen" style={{ background: "linear-gradient(180deg, #060b18 0%, #0c1425 100%)" }}>
      <header className="header-dark">
        <div className="flex items-center gap-3">
          ...
          <span className="font-semibold text-slate-100">学習管理</span>
        </div>
      ...
      <main className="max-w-2xl mx-auto px-4 py-8 space-y-8">
        ...
        <div className="card text-center py-6">
          <p className="text-xs text-slate-600 mb-1">最終学習日</p>
                ...
                return (
                  <div key={p.material_id} className="card">
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <span className="font-medium text-slate-100">{mat?.name ?? "—"}</span>
                        <span className="ml-2 text-xs text-slate-500">{mat?.subject}</span>
                      </div>
                      <span className="font-semibold text-amber-400">{pct.toFixed(0)}%</span>
                    </div>
                ...
                return (
                  <div key={log.id} className="card">
                    <div className="flex items-center justify-between">
                      <div>
                        <span className="font-medium text-slate-300 text-sm">{log.logged_date}</span>
                        {mat && (
                    ...
                    {log.memo && <p className="text-xs text-slate-600 mt-1">{log.memo}</p>}
                  </div>
                );
              })}
            </div>
